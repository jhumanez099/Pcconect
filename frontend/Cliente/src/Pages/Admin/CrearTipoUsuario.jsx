...
export default function CrearTipoUsuario() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // navigate("/admin/consultarTipoUsuario");
    } catch (error) {
      const errorMessage =
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-3">
            <div className="mb-4 row align-items-center">
              ...
              <div className="col-sm-8">
                <input
                  type="text"
                  className={`form-control ${errors.nombre_tipo_usuario ? "is-invalid" : ""}`}
                ...
                {errors.nombre_tipo_usuario && (
                  <div className="invalid-feedback">{errors.nombre_tipo_usuario.message}</div>
                )}
              </div>
            </div>

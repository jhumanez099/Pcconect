    ...
    <tr>
      ...
      <td className="text-end">
        <div className="btn-group btn-group-sm" role="group">
          <button className="btn btn-outline-primary" onClick={() => onEditar(tipoPedido)}>
            <i className="bi bi-pencil-square me-1" /> Editar
          </button>
          <button className="btn btn-outline-danger" onClick={() => onEliminar(tipoPedido)}>
...
export default function ConsultarTipoPedido() {
  ...
  const obtener = async () => {
    try {
      setLoading(true);
      const res = await Axios.get("http://localhost:3000/api/tiposPedidos", { withCredentials: true });
  ...
  const filtrados = useMemo(() => {
    ...
    return tipos.filter((t) => (t.nombre_tipo_pedido || "").toLowerCase().includes(debounced));
  }, [debounced, tipos]);
  ...
  const guardar = async () => {
    try {
      setSaving(true);
      await Axios.put(`http://localhost:3000/api/tiposPedidos/${editing.id_tipo_pedido}`, editing, { withCredentials: true });
      await obtener();
      closeEdit();
    } catch (e) {
      console.error(e);
  ...
  const eliminar = async () => {
    ...
    try {
      setDeleting(true);
      await Axios.delete(`http://localhost:3000/api/tiposPedidos/${toDelete.id_tipo_pedido}`, { withCredentials: true });
      setTipos((prev) => prev.filter((x) => x.id_tipo_pedido !== toDelete.id_tipo_pedido));
      closeConfirm();
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="min-vh-100 d-flex flex-column">
          ...
          <div className="card-body p-4">
            <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-3">
              <div className="d-flex align-items-center gap-2">
                <button className="btn btn-outline-secondary" onClick={() => navigate("/MenuTipoPedido")}>
                  ← Menú Tipo Pedido
                </button>
                <h1 className="h4 mb-0">Consultar tipos de pedido</h1>
              </div>
              ...
              <div className="table-responsive mt-2">
                <table className="table table-hover align-middle">
                  ...
                  <tbody>
                    {filtrados.length > 0 ? (
                      filtrados.map((t) => (
                        <TipoPedidoRow key={t.id_tipo_pedido} tipoPedido={t} onEditar={openEdit} onEliminar={askDelete} />
                      ))
                    ) : (
                      <tr><td colSpan="2" className="text-center text-body-secondary py-4">Sin resultados.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <form>
                <div className="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    className="form-control"
                    value={editing.nombre_tipo_pedido || ""}
      ...
      <Modal isOpen={isConfirmOpen} onRequestClose={closeConfirm} className="custom-modal" overlayClassName="custom-overlay" ariaHideApp={false}>
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">Eliminar tipo de pedido</h5>
            <button type="button" className="btn-close" onClick={closeConfirm} />
          </div>
          <div className="modal-body">
            ¿Eliminar <strong>{toDelete?.nombre_tipo_pedido}</strong>?
          </div>
          <div className="modal-footer">

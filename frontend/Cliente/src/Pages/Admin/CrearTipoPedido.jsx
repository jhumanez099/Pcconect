...
export default function CrearTipoPedido() {
  ...
  const onSubmit = (data) => {
    crearTipoPedido(data);
  };

  const crearTipoPedido = (tipoUsuarioData) => {
    Axios.post("http://localhost:3000/api/tiposPedidos", tipoUsuarioData)
      .then(() => {
        reset();
        setGlobalError(null);
      })
      .catch((error) => {
        const errorMessage =
          error.response && error.response.data && error.response.data.message
            ...
            : "Error al crear el tipo de pedido. Intenta nuevamente.";
        ...
        <div className="col-11 col-sm-10 col-md-8 col-lg-6 col-xl-5 bg-white rounded card shadow p-4 m-4">
          <div className="row my-4 gx-5">
            <div className="col-12 d-flex justify-content-between align-items-center mb-3">
              ...
              <h1 className="text-center w-100 mb-0">Crear tipo pedido</h1>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-3">
            {[
              { label: "Nombre", id: "nombreTipoPedido", type: "text" },
            ].map((field, index) => (
